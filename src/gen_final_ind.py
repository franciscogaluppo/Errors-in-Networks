...
# TODO: só precisa do controle para o tau_param
controles = {
	"Probit": [True, 0, True],
	"Logit": [True, 0, True],
	"Tau-Exposure": [False, 0.85, False],
	"Tau-Exposure-Binario": [False, 0.85, True],
	"Linear": [True, 0, False]
}
...
#zvec = IO.zfile_to_zvector(nome_grafo, 16)
IO.zvector_to_zfile(zvec, nome_grafo)

# Gera valores
predicoes, ATE = Estimate.multiple_estimate(
	g, zvec, ins, betas, model, ests,
	rodadas, modelo_resposta, estoc_params, True, 0.85
)

# Escreve valores
pre_path = ["Resultados", "Novo_Linear", modelo_resposta, nome_grafo]
path = ''
...
IO.ZF_file(g, zvec, "ZF {}".format(nome_grafo))
